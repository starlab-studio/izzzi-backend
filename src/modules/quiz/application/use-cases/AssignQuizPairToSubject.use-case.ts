...
export class AssignQuizPairToSubjectUseCase extends BaseUseCase implements IUseCase {
  constructor(
  ...
  async execute(data: AssignQuizPairToSubjectInput): Promise<AssignQuizPairToSubjectOutput> {
    try {
      ...
      if (existingQuizzes.length > 0) {
        throw new DomainError(
          ...
          "Quizzes already exist for this subject. Use reassign instead.",
        );
      }
      ...
      const duringQuiz = QuizEntity.create({
        subjectId: data.subjectId,
        templateId: templatePair.duringCourseTemplateId,
        type: "during_course",
        accessToken: duringToken,
        publicUrl: duringPublicUrl,
        qrCodeUrl: null, // Will be generated by infrastructure
      });
      duringQuiz.activate();

      const afterQuiz = QuizEntity.create({
        subjectId: data.subjectId,
      ...
      // Activate after_course quiz so it's accessible via public link
      afterQuiz.activate();

      const savedDuringQuiz = await this.quizRepository.create(duringQuiz);
      const savedAfterQuiz = await this.quizRepository.create(afterQuiz);
      ...
      return {
        duringCourse: {
          ...
          template: {
            ...
            name: "Template", // Will be loaded from template repository if needed
          },
        },
        afterCourse: {
