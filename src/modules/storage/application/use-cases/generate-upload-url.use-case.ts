...
import { GenerateUploadUrlCommand, UploadUrlResult } from '../dto/generate-upload-url.dto';
...
export class GenerateUploadUrlUseCase {
  ...
  constructor(
    ...
    @Inject(FILE_KEY_GENERATOR) private readonly fileKeyGenerator: IFileKeyGenerator,
  ...
  async execute(command: GenerateUploadUrlCommand): Promise<UploadUrlResult> {
    try {
      ...
      return new UploadUrlResult(presignedUrl.url, file.key, file.url, presignedUrl.expiresIn, {
