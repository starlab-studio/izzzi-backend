import { IsString, MinLength, IsNotEmpty, IsOptional, IsUUID, IsArray, ValidateNested } from "class-validator";
...
export class BulkCreateSubjectsDto {
  @ApiProperty({
    description: "Tableau de matières à créer",
    type: [BulkCreateSubjectItemDto],
    example: [
      {
        name: "Mathématiques",
        instructorName: "Dr Euler",
        instructorEmail: "euler@example.com",
        firstCourseDate: "2025-01-10",
        lastCourseDate: "2025-06-30",
      },
    ],
  })
  ...
  @ApiProperty({
    description: "ID de la classe à laquelle assigner les sujets",
    example: "123e4567-e89b-12d3-a456-426614174000",
  })
  @IsUUID(4, { message: "Class ID must be a valid UUID" })
  @IsNotEmpty({ message: "Class ID is required" })
